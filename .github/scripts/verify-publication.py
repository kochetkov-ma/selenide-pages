#!/usr/bin/env python3
"""Verify a signed legacy Maven publication before Central upload."""
...
DEPENDENCIES = {
    ("org.jetbrains.kotlin", "kotlin-reflect"): ("1.9.25", "compile"),
    ("org.jetbrains.kotlin", "kotlin-stdlib-jdk8"): ("1.9.25", "compile"),
    ("org.awaitility", "awaitility"): ("4.2.2", "compile"),
    ("org.awaitility", "awaitility-kotlin"): ("4.2.2", "compile"),
    ("io.kotest", "kotest-assertions-core-jvm"): ("5.9.1", "compile"),
    ("org.slf4j", "slf4j-api"): ("2.0.17", "compile"),
    ("io.github.oshai", "kotlin-logging-jvm"): ("7.0.5", "compile"),
    ("io.leangen.geantyref", "geantyref"): ("2.0.1", "compile"),
    ("com.codeborne", "selenide"): ("7.7.3", "compile"),
    ("io.qameta.allure", "allure-java-commons"): ("2.29.1", "compile"),
    ("io.qameta.allure", "allure-selenide"): ("2.29.1", "runtime"),
...
def check_pom(path, version):
    ...
    def value(element, key):
        return (element.findtext(key, default="", namespaces=namespace) or "").strip()
    ...
    expected = {
        ...
        "m:licenses/m:license/m:url": "https://www.apache.org/licenses/LICENSE-2.0.txt",
        ...
        "m:developers/m:developer/m:url": "https://github.com/kochetkov-ma",
        "m:scm/m:url": REPOSITORY_URL,
        "m:scm/m:connection": "scm:git:https://github.com/kochetkov-ma/selenide-pages.git",
        "m:scm/m:developerConnection": "scm:git:ssh://git@github.com/kochetkov-ma/selenide-pages.git",
    }
    for key, wanted in expected.items():
        require(value(root, key) == wanted, f"POM {key} mismatch")
    for key in ("m:name", "m:description"):
        require(value(root, key), f"POM {key} missing")

    actual_dependencies = {}
    for dependency in root.findall("m:dependencies/m:dependency", namespace):
        coordinate = (value(dependency, "m:groupId"), value(dependency, "m:artifactId"))
        require(coordinate not in actual_dependencies, f"Duplicate POM dependency: {':'.join(coordinate)}")
        actual_dependencies[coordinate] = (value(dependency, "m:version"), value(dependency, "m:scope") or "compile")
    require(actual_dependencies == DEPENDENCIES, "POM legacy dependency set or versions differ")
        ...
        try:
            ...
            for signature, artifact in pairs:
                checked = subprocess.run(
                    command + ["--verify", str(signature), str(artifact)],
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                    check=False,
                )
...
def inspect(build, local, version, key):
    require(re.fullmatch(r"1\.4\.0(?:-[A-Za-z0-9][A-Za-z0-9.-]*)?", version), "Invalid legacy publication version")
    ...
    require(
        all(component.get(key) == value for key, value in (("group", GROUP), ("module", ARTIFACT), ("version", version))),
        "Gradle module coordinates mismatch",
    )
...
def main():
    ...
    print(f"Signed legacy publication verified: {GROUP}:{ARTIFACT}:{version}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"Legacy publication verification failed: {error}", file=sys.stderr)
        sys.exit(1)
